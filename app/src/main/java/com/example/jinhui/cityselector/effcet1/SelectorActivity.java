...
//            Log.e(TAG, "run");
    ...
    private void initView() {
        ...
        recyclerView.addItemDecoration(new ItemDecoration(getResources().getDimensionPixelOffset(R.dimen.dp1)));
        mAdapter.notifyDataSetChanged();

        selectorView.setOnChangedListener(new SelectorView.OnChangedListener() {
            @Override
            public void onChanged(String s) {
                mHandler.removeCallbacks(mRunnable); // 移除所有的回调？貌似没什么用，Log.e(TAG, "run");不执行
                tipView.setText(s);
                ...
                for (int i = 0, len = args.length; i < len; i++) {
                    if (args[i].toLowerCase().equals(s.toLowerCase())){
                        /**
                         * scrollBy(x, y)
                         * 这个方法是自己去控制移动的距离，单位是像素,所以在使用scrollBy(x, y)需要自己去计算移动的高度或宽度。
                         * scrollToPosition(position)
                         * 这个方法的作用是定位到指定项，就是把你想显示的项显示出来，但是在屏幕的什么位置是不管的，只要那一项现在看得到了，那它就罢工了！
                         * smoothScrollToPosition(position)
                         * 和scrollToPosition(position)效果基本相似，也是把你想显示的项显示出来，只要那一项现在看得到了，那它就罢工了，不同的是smoothScrollToPosition是平滑到你想显示的项，而scrollToPosition是直接定位显示！
                         * scrollToPositionWithOffset
                         * 主角总是最后才登场，这种方式是定位到指定项如果该项可以置顶就将其置顶显示。比如:微信联系人的字母索引定位就是采用这种方式实现。
                         *
                         * 以上可以参考 https://www.jianshu.com/p/3acc395ae933
                         *
                         */
    ...
    @OnClick({R.id.recyclerView, R.id.selectorView, R.id.tipView})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.recyclerView:
                break;
            case R.id.selectorView:
                break;
            case R.id.tipView:
                break;
        }
